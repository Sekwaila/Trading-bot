...
Bridges streamlit_app.py and worker.py directly to deriv_adapter.py.
"""
...
import deriv_adapter


class DerivClient:
    """Wrapper class around deriv_adapter module to satisfy engine interface."""
    ...
    def get_time_series(self, symbol: str, interval: str = "15m", outputsize: int = 50) -> pd.DataFrame:
        """
        Fetches the live quote via Deriv WebSocket and generates a short OHLC
        series centered at the real tick price so TA indicators run accurately.
        """
        live_price = deriv_adapter.get_deriv_price(symbol)
        
        # Fallback if Deriv WebSocket drops momentarily
...
def generate_omega_signal(
    ...
    try:
        ...
        # Precise entry price directly from latest close (live Deriv quote)
