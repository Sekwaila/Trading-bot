...
Integrates TwelveDataClient with dynamic failover endpoints for Gold, Forex, and Crypto.
"""
...
# Safely import TwelveDataAdapter from twelve_data_adapter.py
try:
...
class DerivClient:
    """
    Primary Data Router: Pulls live quotes from TwelveDataClient first,
    falling back to standard financial REST endpoints if the API key is missing
    or rate-limited on free assets.
    """

    def __init__(self):
        self.api_key = os.getenv("TWELVE_DATA_API_KEY", os.getenv("TWELVEDATA_API_KEY", "")).strip()
        self.td_client = TwelveDataAdapter(self.api_key) if (TwelveDataAdapter and self.api_key) else None
    ...
    def get_live_price(self, symbol: str) -> float:
        ...
        # Step 1: Try TwelveDataClient time series fetch
        if self.td_client:
            try:
                df_td = self.td_client.get_time_series(clean_sym, interval="1min", outputsize=1)
                if df_td is not None and not df_td.empty and "close" in df_td.columns:
                    latest_price = float(df_td["close"].iloc[-1])
                    if latest_price > 0:
                        return latest_price
            except Exception:
                pass

        # Step 2: Live Yahoo Finance REST Fallback (Direct Real-Time Quote)
    ...
    def get_time_series(self, symbol: str, interval: str = "15m", outputsize: int = 50) -> pd.DataFrame:
        ...
        # Attempt fetch from Twelve Data first
        if self.td_client:
            try:
                df_td = self.td_client.get_time_series(clean_sym, interval=interval, outputsize=outputsize)
                if df_td is not None and not df_td.empty and len(df_td) >= 10:
                    return df_td
            except Exception:
                pass

        # Fallback to dynamic OHLC built directly on current live price tick
        ...
        close_prices[-1] = live_price  # Force last candle close to live MT quote
