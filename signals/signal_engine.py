...
class SignalEngine:
    ...
    def generate_signal(self, df):
        ...
        premium_data = premium_discount.analyze(df)
        if premium_data is None:
            return None

        breaker_data = breaker_blocks.analyze(df)
        ...
        mitigation_data = mitigation_blocks.analyze(df)
        if mitigation_data is None:
            return None

        inducement_data = inducement.analyze(df)
        if inducement_data is None:
            return None

        displacement_data = displacement.analyze(df)
        if displacement_data is None:
            return None

        session_data = sessions.analyze(df)
        if session_data is None:
            return None

        mtf_data = multi_timeframe.analyze(df)
        if mtf_data is None:
            return None
        ...
        # Confirmed keys: "bullish_mitigation" / "bearish_mitigation"
        # Mirrors the order_blocks / breaker_blocks weighting.
        # =====================================
        ...
        # =====================================
        # Inducement (V10)
        # Confirmed keys: "bullish_inducement" / "bearish_inducement"
        # Weighted lower — inducement is typically a manipulation/trap
        # signal, not a high-conviction directional one on its own.
        # =====================================
        ...
        # =====================================
        # Displacement (V10)
        # Confirmed keys: "bullish_displacement" / "bearish_displacement"
        # Weighted high — a strong displacement move is normally treated
        # as a high-conviction directional signal in SMC systems.
        # =====================================
        ...
        # =====================================
        # Sessions (V10)
        # Confirmed shape: {"asian": bool, "london": bool,
        #                    "new_york": bool, "active_session": str}
        # Not directional on its own — scored as an equal confidence
        # bonus to both sides when a major session (London or New York)
        # is active. London/NY get the bonus; Asian is typically lower
        # volatility for most SEKWAILA instruments (XAUUSD/US30/EURUSD/
        # SP500/USDJPY) so it's excluded. Adjust if you want Asian
        # session included (e.g. for USDJPY specifically).
        # =====================================
        ...
        # Confirmed keys: "bullish_alignment" / "bearish_alignment"
        # (module also returns current_trend/h1_trend/h4_trend strings,
        # not used in scoring — included in the output dict for
        # display/logging.)
        # Weighted heaviest of the new modules — HTF alignment is
        # typically the strongest confluence factor in SMC systems.
        # =====================================
