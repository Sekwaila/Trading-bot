...
Combines SMC (LuxAlgo style Order Blocks, FVG, MSS) with Multi-Timeframe RSI Alignment.
"""
...
# --------------------------------------------------------------------
# 1. Technical Indicators & SMC Calculators
# --------------------------------------------------------------------
...
def detect_smc_structures(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Detects LuxAlgo Smart Money Concepts:
    - Fair Value Gap (FVG)
    ...
    # 1. Fair Value Gap (FVG) Check (Last 3 candles)
    fvg_bullish = low[-1] > high[-3]  # Gap up
    fvg_bearish = high[-1] < low[-3]  # Gap down

    # 2. Order Block Detection (Last swing low/high candle before impulse)
...
def analyze_market(symbol: str, timeframe: str, client: Any) -> Dict[str, Any]:
    """
    Fetches market candles, calculates SMC + RSI alignment, and scores confidence (0-100%).
    """
    try:
        # Fetch OHLCV data via TwelveData adapter
        df = client.get_time_series(symbol=symbol, interval=timeframe, outputsize=50)

        if df is None or df.empty or len(df) < 15:
            return {"ok": False, "reason": "Insufficient candles or API timeout."}
        ...
        # Multi-timeframe trend placeholders (Simulated or fetched)
        tf_alignment = {
...
# --------------------------------------------------------------------
# 3. Market Scanner Overview (Ordered Ranking)
# --------------------------------------------------------------------
def get_market_overview(watchlist: List[str], client: Any) -> pd.DataFrame:
    """Scans and ranks multiple pairs from strongest BUY down to strongest SELL."""
    results = []
    ...
    if not df.empty:
        # Sort by raw score descending (Strongest Buy -> Strongest Sell)
