...
def fetch_mtf_data(symbol):
    ...
    if websockets is None:
        reason = "UNAVAILABLE (the 'websockets' package is not installed — add it to requirements.txt)"
        logger.error(reason)
        return {tf: None for tf in DERIV_GRANULARITY}, {tf: reason for tf in DERIV_GRANULARITY}

    symbol_code = DERIV_SYMBOL_MAP.get(symbol)
    if not symbol_code:
        reason = f"UNAVAILABLE (no Deriv symbol mapped for {symbol})"
        logger.error(reason)
        return {tf: None for tf in DERIV_GRANULARITY}, {tf: reason for tf in DERIV_GRANULARITY}
...
def generate_omega_signal(symbol,ticker,min_tf=DEFAULT_MIN_TF_AGREEMENT,min_score=DEFAULT_MIN_SCORE,min_rr=DEFAULT_MIN_RR):
    # `ticker` (the yfinance code from config.ASSETS) is kept in the signature so
    # streamlit_app.py doesn't need to change its call site, but it's unused now —
    # the live feed is Deriv, resolved from `symbol` via DERIV_SYMBOL_MAP.
