...
Does NOT depend on data.market_data or local missing files.
"""
...
def calculate_risk_levels(price: float, is_buy: bool) -> Dict[str, Any]:
    """Calculate structured Stop-Loss, Take-Profits, and Risk-Reward ratios."""
    if price <= 0:
        return {
            "sl": 0.0,
            "tp1": 0.0,
    ...
    # Standard percentage-based targets for multi-market stability
    sl_dist = price * 0.005  # 0.5% Risk
...
def analyze_market(
    ...
    """Unified entry point for market analysis."""
    default_response = {
        "ok": False,
        "symbol": symbol,
        "bias": "NEUTRAL",
    ...
    if not client_adapter:
        default_response["reason"] = "Market data adapter not provided"
        return default_response

    candles, err = client_adapter.get_candles(symbol, timeframe, outputsize=50)
    if err or not candles:
        default_response["reason"] = f"API Error: {err}"
        return default_response

    try:
        closes = [float(c["close"]) for c in candles if "close" in c]
        if len(closes) < 20:
            default_response["reason"] = "Insufficient candle volume"
            return default_response
        ...
        if recent_avg > older_avg:
            ...
            is_buy = True
            reason = "Price momentum expanding upward."
        elif recent_avg < older_avg:
            diff = (older_avg - recent_avg) / older_avg
            score = max(5, int(45 - (diff * 2000)))
            is_buy = False
            reason = "Price momentum expanding downward."
        else:
            score = 50
            is_buy = True
            reason = "Price consolidated near equilibrium."
...
def get_market_overview(
    ...
    """Constructs a consolidated market table."""
    records = []
    if not client_adapter:
