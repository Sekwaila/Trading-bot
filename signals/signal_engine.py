...
SMC voting engine with flexible detector wrappers and strong EMA fallback.
"""

import numpy as np
import pandas as pd

# ---------- SMC imports (each individually) ----------
SMC_MODULES = {}
...
try:
    from .fair_value_gap import FVG  # adjust class name if needed
    SMC_MODULES['fvg'] = FVG
except ImportError:
    pass

try:
    from .order_blocks import OrderBlocks  # adjust class name if needed
...
class SignalEngine:
    ...
    def __init__(self):
        ...
        if not SMC_AVAILABLE:
            print("ℹ️  No SMC modules found – falling back to EMA+RSI only.")
    ...
    def generate_signal(self, df, symbol="EUR/USD", timeframe="1H"):
        ...
        if self.smc_available:
            ...
            # Add indicators for proper SL/TP
            df = self._add_indicators(df)
            return self._build_signal(df, direction, confidence)

        # ------------------ Fallback to EMA/RSI ------------------
        print("➡️  SMC not available – using EMA/RSI")
        return self._ema_rsi_signal_with_levels(df)

    def _ema_rsi_signal(self, df):
        """
        Return BUY/SELL/None based on EMA trend + RSI confirmation.
        Less strict than crossover – signals during established trends.
        """
        df = self._add_indicators(df)
        last = df.iloc[-1]
    ...
    def _ema_rsi_signal_with_levels(self, df):
        """EMA/RSI logic with full signal object."""
        df = self._add_indicators(df)
        direction = self._ema_rsi_signal(df)
        if direction is None:
            print("➡️  EMA/RSI: no clear signal")
            return None
