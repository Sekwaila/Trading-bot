...
Step 1:
- Signal computation remains based on the provider data in ASSETS.
- No calibration/offset is applied inside the engine.
- Entry remains the last CLOSED candle by design.
- The live quote is a separate display concern.
"""
...
def fetch_usdzar_rate():
    try:
        d = _clean_ohlcv(yf.download(
            "ZAR=X", period="5d", interval="1d",
            progress=False, auto_adjust=False, threads=False,
        ))
        if d.empty:
            return None
...
def compute_adx(df, length=14):
    ...
    return _safe_float(
        dx.ewm(alpha=1/length, adjust=False, min_periods=length).mean().iloc[-1],
        20.0,
    )
...
def compute_market_regime(df):
    ...
    regime = (
        "TRENDING_EXPANSION" if adx >= 25 and vol >= 1.10
        else "ACCUMULATION_DISTRIBUTION" if adx < 20 and vol < .85
        else "HIGH_VOLATILITY_RANGE" if vol >= 1.40
        else "CHOP_LOW_VOLATILITY"
    )
    return {
        "regime":regime, "adx":round(adx,2),
        "vol_ratio":round(vol,2), "slope":round(_safe_float(slope),4),
    }
...
def detect_choch(df):
    ...
    return {
        ...
        "bias":bias, "swing_high":sh, "swing_low":sl,
        "structure":structure,
    }
...
def find_order_block(df,bias):
    d = _closed(df)
    neutral_zone = (
        (float(d["Low"].tail(10).min()),float(d["High"].tail(10).max()))
        if not d.empty else (0.0,0.0)
    )
    ...
    for i in range(max(10,n-100),n-4)[::-1]:
        op,cl,hi,lo = [
            _safe_float(d[c].iloc[i]) for c in ("Open","Close","High","Low")
        ]
...
def detect_fvg(df,lookback=FVG_LOOKBACK):
    ...
    for i in range(start,len(d)-1):
        ...
        future=d.iloc[i+2:]
        filled=False if future.empty else bool(
            ((future["Low"]<=zone[1])&(future["High"]>=zone[0])).any()
        )
...
def calculate_premium_discount(df,lookback=50):
    d=_closed(df)
    if d.empty:
        return {"zone":"UNKNOWN","equilibrium":0.0,"swing_high":0.0,"swing_low":0.0}
    d=d.tail(lookback)
    hi=_safe_float(d["High"].max()); lo=_safe_float(d["Low"].min())
    eq=(hi+lo)/2; cl=_safe_float(d["Close"].iloc[-1])
    return {
        "zone":"PREMIUM" if cl>eq else "DISCOUNT" if cl<eq else "EQUILIBRIUM",
...
def evaluate_trend_strength(d,tf_biases,regime_info,struct_bias):
    ...
    if bull and adx_ok and bc>=3 and struct_bias=="BUY":
        return True,"EMA stack + ADX + 3/4 TF aligned bullish"
    if bear and adx_ok and sc>=3 and struct_bias=="SELL":
...
def calculate_position_size(account_balance_usd,risk_pct,entry_price,stop_loss_price,contract_size=100.0):
    vals=[_safe_float(x) for x in (
        account_balance_usd,risk_pct,entry_price,stop_loss_price,contract_size
    )]
    ...
    return {
        ...
        "lots":round(risk_amount/(dist*contract),4),
    }
...
def calculate_position_size_for_symbol(symbol,account_balance_usd,risk_pct,entry_price,stop_loss_price):
    ...
    result=calculate_position_size(
        account_balance_usd,risk_pct,entry_price,stop_loss_price,contract
    )
...
def generate_omega_signal(symbol,ticker,min_tf=DEFAULT_MIN_TF_AGREEMENT,min_score=DEFAULT_MIN_SCORE,min_rr=DEFAULT_MIN_RR):
    ...
    if unavailable:
        return {
            "ok":False,"symbol":symbol,"ticker":ticker,
            "reason":"Required timeframe data unavailable: "+", ".join(unavailable),
    ...
    if atr<=0:
        return {
            "ok":False,"symbol":symbol,"ticker":ticker,
            "reason":"ATR could not be calculated.","data_integrity":integrity,
        }
    ...
    if struct_bias=="BUY":
        ...
        stop=min(ref-buffer,entry-atr_floor)
        tp1=entry+1.5*atr; tp2=entry+3*atr; tp3=entry+5*atr
    elif struct_bias=="SELL":
        ref=ob_zone[1] if ob_type=="BEARISH_OB" and not ob_invalidated else sw_high
        if ref is None: ref=entry+atr*1.5
        stop=max(ref+buffer,entry+atr_floor)
        tp1=entry-1.5*atr; tp2=entry-3*atr; tp3=entry-5*atr
    else:
    ...
    if bc>=min_tf and score>=min_score and struct_bias=="BUY":
        bias="BUY"
    elif sc>=min_tf and score>=min_score and struct_bias=="SELL":
        bias="SELL"
    else:
        bias="NEUTRAL"
        reason=f"Timeframe agreement or score below threshold ({max(bc,sc)}/{min_tf} TF, {score}/{min_score} score)"

    if bias!="NEUTRAL" and rr<min_rr:
        bias="NEUTRAL"
        reason=f"R:R {rr:.2f} below minimum {min_rr:.2f} — signal downgraded to NEUTRAL"
    ...
    return {
        ...
        "data_integrity":integrity,"bias":bias,"score":score,
        "grade":grade(score),"entry":entry,"stop":stop,"tp1":tp1,
        "tp2":tp2,"tp3":tp3,"rr":round(rr,3),"atr":round(atr,4),
        "tf_biases":biases,"tf_structures":structures,
        "structure":struct_type,"ob_type":ob_type,"ob_zone":ob_zone,
        "ob_mitigated":ob_mitigated,"ob_invalidated":ob_invalidated,
        "fvg":fvg,"sweep":sweep,"sweep_detail":sweep_detail,
        "pd_zone":pd_info["zone"],"pd_info":pd_info,"session":session,
        "session_quality":session_quality,"regime":regime,
        "trend_strong":trend_strong,"trend_detail":trend_detail,
        "eq_highs":eq_highs,"eq_lows":eq_lows,"rsi":round(rsi,2),
        "macd_trend":macd_trend,"macd_line":macd_line,
        "macd_signal":macd_signal,"vwap_status":vwap_status,
        "vwap_val":vwap_value,"vol_status":vol_status,"ema_cross":ema_cross,
        "bull_score":bull_score,"bear_score":bear_score,
        "bull_tf_count":bc,"bear_tf_count":sc,"reason":reason,
    }
