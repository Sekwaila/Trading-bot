...
Reliable Yahoo Finance fetching, per-timeframe availability, no calibration.py dependency.
"""
...
_YF_MIN_RETRY_SECONDS = 20
_YF_MAX_RETRIES = 2


def _is_rate_limit_error(exc: Exception) -> bool:
    text = str(exc).lower()
    return any(x in text for x in ("ratelimit", "rate limit", "too many requests", "429", "throttle"))
...
def _clean_ohlcv(df: Any) -> pd.DataFrame:
    ...
    if isinstance(df.columns, pd.MultiIndex):
        ...
        df.columns = level0 if all(c in level0 for c in ("Open", "High", "Low", "Close")) else df.columns.get_level_values(-1)
...
def _download_yahoo(ticker: str, period: str, interval: str, retries: int = _YF_MAX_RETRIES) -> pd.DataFrame:
    last_exc = None
    for attempt in range(retries + 1):
        try:
            ...
            if _is_rate_limit_error(exc) and attempt < retries:
                wait_s = _YF_MIN_RETRY_SECONDS * (attempt + 1)
                logger.warning("%s Yahoo rate limit on %s/%s; retrying in %ss",
                               ticker, period, interval, wait_s)
                time.sleep(wait_s)
                continue
            logger.warning("%s Yahoo download failure on %s/%s: %s",
                           ticker, period, interval, exc)
            return pd.DataFrame()
    if last_exc:
        logger.warning("%s Yahoo download failed on %s/%s: %s",
                       ticker, period, interval, last_exc)
    return pd.DataFrame()
...
def _resample_4h(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame()
    try:
        return _clean_ohlcv(df.resample("4h").agg({
            "Open": "first", "High": "max", "Low": "min",
            "Close": "last", "Volume": "sum"
        }).dropna(subset=["Open", "High", "Low", "Close"]))
    except Exception as exc:
        logger.warning("4H resampling failed: %s", exc)
        return pd.DataFrame()


def _validate_timeframe(tf_label: str, df: pd.DataFrame) -> Tuple[bool, str]:
    if df is None or df.empty:
        return False, "UNAVAILABLE (Yahoo returned no data)"
...
def fetch_mtf_data(ticker):
    ...
    for tf_label, (period, interval) in TF_CONFIG.items():
        try:
            ...
            if tf_label == "4H" and not df.empty:
                df = _resample_4h(df)
            ok, status = _validate_timeframe(tf_label, df)
            if not ok:
                tf_data[tf_label] = None
                integrity[tf_label] = status
                logger.warning("%s %s data failure: %s", ticker, tf_label, status)
            else:
                tf_data[tf_label] = df
...
def fetch_usdzar_rate():
    try:
        d = _download_yahoo("ZAR=X", "5d", "1d", retries=1)
        if d.empty:
            return None
...
def compute_live_correlation_matrix():
    ...
    for name, ticker in ASSETS.items():
        try:
            d = _download_yahoo(ticker, "10d", "1h", retries=0)
            if not d.empty:
                closes[name] = d["Close"]
        except Exception as exc:
...
def generate_omega_signal(symbol,ticker,min_tf=DEFAULT_MIN_TF_AGREEMENT,min_score=DEFAULT_MIN_SCORE,min_rr=DEFAULT_MIN_RR):
    ...
    if "15M" not in available:
        return {"ok":False,"symbol":symbol,"ticker":ticker,
                "reason":"Primary timeframe 15M unavailable. No signal generated.",
                "data_integrity":integrity,"available_timeframes":list(available),"unavailable_timeframes":unavailable}
    ...
    if closed.empty:
        return {"ok":False,"symbol":symbol,"ticker":ticker,"reason":"No closed 15M candle available.","data_integrity":integrity}

    entry=_safe_float(closed["Close"].iloc[-1]); atr=compute_atr(closed)
    if atr<=0:
        return {"ok":False,"symbol":symbol,"ticker":ticker,"reason":"ATR could not be calculated.","data_integrity":integrity}
    ...
    effective_min_tf=min(max(int(min_tf),1),max(len(biases),1)); reason=None

    if bc>=effective_min_tf and score>=min_score and struct_bias=="BUY": bias="BUY"
    elif sc>=effective_min_tf and score>=min_score and struct_bias=="SELL": bias="SELL"
    else:
        bias="NEUTRAL"
        reason=f"Timeframe agreement or score below threshold ({max(bc,sc)}/{effective_min_tf} usable TF, {score}/{min_score} score)"
