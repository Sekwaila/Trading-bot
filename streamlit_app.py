"""
SEKWAILA OMEGA X v6.2 – Professional Dashboard
...
# ------------------ LOGGING (writes to file, NOT to UI) ------------------
logging.basicConfig(
    filename="omega.log",
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    force=True  # Override any existing console handlers
)
# Ensure no console output from the root logger
...
# Try spot first (silently), then fallback to futures
YFINANCE_MAP = {
...
# RSI filter thresholds (optional)
RSI_OVERBOUGHT = 70
RSI_EXTREME = 75
...
# ------------------------------------------------
# save_signal now keeps only the latest signal per symbol
# ------------------------------------------------
...
def fetch_from_yfinance(symbol, timeframe=TIMEFRAME, bars=200):
    ...
    for ticker in candidates:
        ...
        if df.empty:
            # Only warn if it's not XAUUSD=X (to reduce noise)
            if ticker != "XAUUSD=X":
                st.warning(f"{symbol}: Yahoo ticker {ticker} failed — {reason}")
            else:
                # Silent fail for XAUUSD=X
                pass
            continue
...
for symbol in SYMBOLS:
    ...
    if rsi is not None:
        if signal["signal"] == "BUY":
            if rsi > RSI_EXTREME:
                st.info(f"{symbol}: BUY signal rejected (RSI={rsi:.1f} > {RSI_EXTREME})")
                continue
            elif rsi > RSI_OVERBOUGHT:
                # Reduce confidence
                signal["confidence"] = max(0, signal["confidence"] - 10)
                # log in file (not UI)
                logging.info(f"RSI filter: {symbol} BUY confidence reduced from {signal['confidence']+10} to {signal['confidence']} (RSI={rsi:.1f})")
        # For SELL, you could add an oversold filter if desired
