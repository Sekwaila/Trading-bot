...
# Compute root directory and Signals package directory
ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
SIGNALS_DIR = os.path.join(ROOT_DIR, "Signals")

# Explicitly insert both into Python's module search path
...
# Safe import fallback for module resolution across host environments
try:
...
# ----------------- MAIN DASHBOARD VIEW -----------------
    ...
    if signal["status"] == "ERROR":
        ...
        st.warning("If hosted on Streamlit Cloud (Linux), direct MT5 terminal connection is unavailable. Run locally on Windows or utilize an external API bridge for MT5 live feeds.")
    else:
        st.markdown(f"""
