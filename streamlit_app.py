"""
SEKWAILA OMEGA X v6
Professional Trading Dashboard – Uses yfinance for market data.
"""
...
def fetch_candles(symbol, timeframe=TIMEFRAME, bars=200):
    ...
    # Estimate period (bars * interval duration in days)
    duration_per_bar = {
    ...
    try:
        ...
        if df.empty:
            return generate_demo_candles(symbol, bars)
        df = df.tail(bars)
        df.index = pd.to_datetime(df.index)
        # Rename columns to match engine's expectations (lowercase)
        df.rename(columns={
            'Open': 'open',
            'High': 'high',
            'Low': 'low',
...
def generate_demo_candles(symbol, bars=200):
...
for symbol in SYMBOLS:
    ...
    if diag:
        ...
        st.write(f"**MACD:** {diag.get('macd_hist', 'N/A')}")
