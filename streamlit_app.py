...
def fetch_candles(symbol, timeframe=TIMEFRAME, bars=200):
    ...
    # For 1H, 5d gives about 120 candles, so we use 10d or 1mo for safety
...
def get_current_price(symbol):
    try:
        ticker = get_yahoo_ticker(symbol)
        # Use 1 day of 1-minute data to get the latest price
        data = yf.download(ticker, period="1d", interval="1m", progress=False)
        if not data.empty:
            # Direct access without float() conversion
...
class SignalEngine:
    ...
    def generate_signal(self, df, symbol="EUR/USD", timeframe="1H"):
        ...
        buffer = 0.01
        entry = price - atr * buffer if direction == "BUY" else price + atr * buffer
        ...
        grade = "A" if confidence >= 85 else "B" if confidence >= 70 else "C" if confidence >= 60 else "D"
    ...
    for col, item in zip(cols, prices):
        with col:
            if item["success"] and item["price"] is not None:
                try:
                    # item["price"] is a float or a pandas Series; ensure float
