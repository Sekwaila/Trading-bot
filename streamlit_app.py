...
# =============================================================================
# 4. MARKET DATA PROVIDER ENGINE (Deterministic Demo + Live Integration)
# =============================================================================
TF_YFINANCE_MAP = {
...
def fetch_market_data(symbol_alias, timeframe):
    ...
    try:
        ...
        if df.empty:
            st.warning(f"No live data returned for {symbol_alias}. Falling back to deterministic demo data.")
...
def calculate_sekwaila_signal(df):
    ...
    # 3. Market Structure & MFI Pressure
    if last['MFI'] > 50:
        structure_score += 25
    ...
    # 4. Volatility & Trend Strength (ADX)
    if last['ADX'] > 25:
        volatility_score += 25
...
# =============================================================================
# 7. INTERACTIVE CHARTING ENGINE (PLOTLY CANDLESTICK + OVERLAYS)
# =============================================================================
def render_professional_chart(df, symbol, timeframe, signal_data, overlays):
    ...
    # 3. Market Structure Annotations & FVGs
    ...
    # 4. Signal Entry / SL / TP Horizontal Lines
...
def render_chart_page():
    ...
    with tb1:
        all_symbols = [alias for category in st.session_state.watchlist.values() for alias in [SYMBOL_ALIASES.get(s, s) for s as s in category]]
        symbol = st.selectbox("Asset", sorted(list(set(all_symbols))), index=0 if st.session_state.selected_symbol not in all_symbols else sorted(list(set(all_symbols))).index(st.session_state.selected_symbol))
    ...
    # Main Workspace Layout (Chart Center, Signal/Analysis Right)
    ...
    with chart_col:
        ...
        # Indicator Dashboard Grid Below Chart
        st.markdown(f"""
        <div class="term-card">
...
def render_katlego_ai_page():
    ...
    # Render Katlego Automated Analysis Card
    st.markdown(f"""
    <div class="term-card-gold">
    ...
    # Katlego Interactive Chat Interface
    ...
    if user_input or query:
        ...
        # Rule-based response engine using live computed market data
...
def render_market_scanner_page():
    ...
    all_symbols = [alias for category in st.session_state.watchlist.values() for alias in [SYMBOL_ALIASES.get(s, s) for s in category]]
    
    scanner_results = []
    progress_bar = st.progress(0)
    
    for idx, sym in enumerate(set(all_symbols)):
        ...
        progress_bar.progress((idx + 1) / len(set(all_symbols)))
    progress_bar.empty()
...
def render_mtf_page():
    ...
    alignment = max(bullish_count, bearish_count) / len(timeframes) * 100
...
def render_journal_page():
    ...
    # Load and Display Persistent Entries
    conn = sqlite3.connect(DB_FILE)
...
def render_settings_page():
    ...
    with t3:
        ...
        wp_url = st.text_input("Wallpaper Image URL (Supports custom ancestor/family artwork)", value=st.session_state.get("wallpaper_bg", ""))
