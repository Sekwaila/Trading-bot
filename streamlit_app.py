...
# Yahoo Finance uses different tickers entirely. Each symbol now maps to an
# ORDERED LIST of candidates — tried in sequence, first one that returns sane
# data wins. XAUUSD=X (spot-tracking) is tried first for gold since it should
# track your MT5 broker's XAUUSDm quote far more closely than futures; GC=F
# (COMEX futures) is the second attempt if XAUUSD=X fails. Last time XAUUSD=X
# returned empty silently — fetch_from_yfinance now logs the actual reason
# for each failed candidate instead of swallowing it, so a repeat failure is
...
# Sanity guard: how far a fallback price is allowed to drift from the last
# trusted price (most recent candle already in the local SQLite cache) before
# it's rejected outright. Set to 1% — tested against the actual GC=F/XAUUSDm
# gap observed (~1.4%, $58 on a ~$4165 base): a 2% threshold would NOT have
# caught it, so 1% is the meaningful floor here. This is a backstop, not the
# primary fix — the primary fix is trying XAUUSD=X (spot-tracking) before
# GC=F (futures) in YFINANCE_MAP above, since the real problem was an
# instrument mismatch, not random bad data. Only applied to the Yahoo
# fallback path, not Twelve Data — a genuine fast move on the primary
# source is trusted; a jump on the fallback path is treated with suspicion,
# since that path is already a known imperfect proxy.
...
def _is_price_sane(symbol, timeframe, new_price, max_deviation_pct=MAX_FALLBACK_DEVIATION_PCT):
    """Compare a candidate fallback price against the last trusted candle
    already sitting in the local cache. Returns (is_sane, last_price_or_None,
    deviation_pct_or_None) so callers can log a specific, useful reason
    instead of a bare rejection."""
    last_cached = load_cached_candles(symbol, timeframe, bars=1)
    if last_cached.empty:
        # Nothing to compare against yet (first run, empty cache) — let it
        # through rather than blocking forever on a cold start.
...
def _normalize_yf_dataframe(data, timeframe, bars):
    """Shared normalization for whichever yfinance call produced `data`.
    Returns (df, reason) — same contract as _fetch_one_yfinance_ticker."""
...
def _fetch_one_yfinance_ticker(ticker, timeframe, bars):
    """Fetch+normalize a single Yahoo ticker. Returns (df, reason) — df is
    empty and reason explains why on any failure. Never raises.

    Tries two different Yahoo access methods for the same ticker before
    giving up: yf.download() and yf.Ticker().history() hit different Yahoo
    endpoints under the hood and don't always agree on which tickers have
    data available — XAUUSD=X returning consistently empty via download()
    doesn't necessarily mean it's empty via the Ticker().history() path."""
...
def fetch_from_yfinance(symbol, timeframe=TIMEFRAME, bars=200):
    """Best-effort fallback when Twelve Data is unavailable. Tries each
    candidate ticker for this symbol in order, logging why each failed
    attempt was rejected (empty data, or price too far from the last
    trusted candle) so a repeat failure is debuggable instead of a
    silent 'No candle data'. Returns (df, ticker_used) — df is empty and
    ticker_used is None if every candidate failed."""
    if not YFINANCE_AVAILABLE:
        return pd.DataFrame(), None

    candidates = YFINANCE_MAP.get(symbol, [])
    if isinstance(candidates, str):
        candidates = [candidates]  # tolerate old single-ticker config style
