...
VERSION = "6.0"
SYMBOLS = ["XAU/USD", "BTC/USD", "EUR/USD"]   # Twelve Data symbol format
TIMEFRAME = "1H"
...
if not TWELVE_API_KEY:
    st.error("🚨 Missing Twelve Data API key. Please add TWELVE_API_KEY in Streamlit Secrets.")
...
def fetch_candles(symbol, timeframe=TIMEFRAME, bars=200):
    ...
    try:
        ...
        if "values" not in data:
            st.warning(f"Twelve Data error for {symbol}: {data.get('status', 'unknown')}")
...
@st.cache_data(ttl=10)   # cache price for 10 seconds
...
from signals.signal_engine import engine   # your existing signal_engine.py
