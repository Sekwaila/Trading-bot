...
# STEP 2: Removed XAUUSD=X – only GC=F for gold
YFINANCE_MAP = {
    "XAU/USD": ["GC=F"],
    "BTC/USD": ["BTC-USD"],
    "EUR/USD": ["EURUSD=X"],
...
def _fetch_one_yfinance_ticker(ticker, timeframe, bars):
    ...
    # STEP 3: reduced period from 60d to 30d for better reliability
...
# ------------------ COMBINED FETCH (cache TTL = 60 seconds) ------------------
...
with st.sidebar:
    ...
    st.info(f"Data cached 1 min | Twelve Data → Yahoo Finance fallback")
    if not YFINANCE_AVAILABLE:
...
sources = []
for sym in SYMBOLS:
    _, src = fetch_candles(sym, TIMEFRAME)
    sources.append(src)
