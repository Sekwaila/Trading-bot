...
<style>
    ...
    .stButton>button {
        background-color: #2962ff; color: white; border-radius: 4px; border: none; font-weight: 600; width: 100%;
    }
...
if "tf_selected" not in st.session_state:
    st.session_state["tf_selected"] = "1H"
...
class KatlegoAIEngine:
    ...
    def evaluate_signal(df):
        if len(df) < 50:
            return "NEUTRAL", ["Insufficient candle data"], 0
...
# ==========================================
# 4. FIXED MARKET DATA GENERATOR
# ==========================================
@st.cache_data(ttl=60)
def fetch_market_data(symbol, timeframe, bars=100):
    np.random.seed(42)
    
    # Map timeframe selection to compatible Pandas frequency offset codes
    freq_map = {
        "15m": "15min",
        "1H": "1h",
        "4H": "4h",
    ...
    selected_freq = freq_map.get(timeframe, "1h")
    
    dates = pd.date_range(end=datetime.now(), periods=bars, freq=selected_freq)
...
if tf_cols[0].button("M15"): st.session_state["tf_selected"] = "15m"
if tf_cols[1].button("1H"):  st.session_state["tf_selected"] = "1H"
if tf_cols[2].button("4H"):  st.session_state["tf_selected"] = "4H"
if tf_cols[3].button("D1"):  st.session_state["tf_selected"] = "1D"

active_tf = st.session_state["tf_selected"]
st.sidebar.info(f"Active Timeframe: **{active_tf}**")
...
# ==========================================
st.title(f"📊 {symbol} — {active_tf} Trading Dashboard")

# Fetch Data safely using session state timeframe
raw_data = fetch_market_data(symbol, active_tf)
