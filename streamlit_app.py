...
Step 1 price architecture:
- Manual calibration is no longer applied to signal levels or live prices.
- The dashboard uses symbol-based live-price requests.
- Signal computation remains in signals/signal_engine.py.
- Yahoo Finance remains the current data provider; MT5/broker integration is
...
if "view_symbol" not in st.session_state:
    st.session_state.view_symbol = None
...
def _enforce_data_integrity(result):
    """
    UI safety gate:
    Never allow the dashboard to display or count a signal when any required
    timeframe is unavailable. This is deliberately enforced here even if an
    older/buggy signal engine returns partial data.
    """
    if not isinstance(result, dict):
        return {
            "ok": False,
            "reason": "DATA UNAVAILABLE — signal engine returned no valid result.",
            "data_integrity": {},
        }

    integrity = result.get("data_integrity") or {}
    data = result.get("data") or {}

    unavailable = []
    for tf in TF_ORDER:
        status = str(integrity.get(tf, "")).upper()
        frame = data.get(tf)
        ...
        if status and not status.startswith("LIVE"):
            unavailable.append(tf)

    # Deduplicate while preserving TF_ORDER.
...
def _fetch_one(symbol, ticker, min_tf, min_score, min_rr, _bucket):
    try:
        ...
        return _enforce_data_integrity(result)
    except Exception as exc:
        return _enforce_data_integrity({
            "ok": False,
            "symbol": symbol,
            "ticker": ticker,
...
if st.session_state.view == "settings":
    ...
    with tabs[0]:
        ...
        S["general"]["refresh_seconds"] = st.selectbox(
            ...
            index=[30, 60, 120, 300].index(S["general"]["refresh_seconds"])
            if S["general"]["refresh_seconds"] in [30, 60, 120, 300] else 2,
        )
        S["general"]["account_currency"] = st.selectbox(
            "Account currency", ["ZAR", "USD"],
            index=["ZAR", "USD"].index(S["general"]["account_currency"]),
        )
        S["general"]["account_balance"] = st.number_input(
            f"Account balance ({S['general']['account_currency']})",
            min_value=0.0, value=float(S["general"]["account_balance"]), step=500.0,
        )

    with tabs[1]:
        st.subheader("Signal engine thresholds")
        st.caption("These feed directly into generate_omega_signal().")
        S["signals"]["min_tf_agreement"] = st.slider(
            "Minimum timeframe agreement", 1, len(TF_ORDER),
            int(S["signals"]["min_tf_agreement"]),
        )
        S["signals"]["min_score"] = st.slider(
            "Minimum score", 0.0, 100.0, float(S["signals"]["min_score"]),
        )
        S["signals"]["min_rr"] = st.number_input(
            "Minimum R:R", 0.1, 10.0, float(S["signals"]["min_rr"]), 0.1,
        )
        st.caption(
            "Classification tiers are set in config.py — EXTREME_SCORE_MIN, "
            "STRONG_SCORE_MIN, WEAK_SCORE_MAX, EXTREME_MIN_TF_AGREEMENT."
        )

    with tabs[2]:
        st.subheader("Risk")
        S["risk"]["risk_pct"] = st.slider(
            "Risk per trade (%)", 0.10, 5.00, float(S["risk"]["risk_pct"]), 0.10,
        )

    with tabs[3]:
        st.subheader("AI")
        st.caption(
            "The project does not currently call an AI provider. This section "
            "stores configuration for future integration only."
        )
        S["ai"]["enabled"] = st.toggle("AI analysis enabled", value=bool(S["ai"]["enabled"]))
        S["ai"]["provider"] = st.selectbox(
            "Provider", ["Anthropic", "OpenAI", "Other"],
            index=["Anthropic", "OpenAI", "Other"].index(S["ai"]["provider"])
            if S["ai"]["provider"] in ["Anthropic", "OpenAI", "Other"] else 0,
        )
        ...
        S["ai"]["confidence_threshold"] = st.slider(
            "Minimum AI confidence to surface", 0.0, 100.0,
            float(S["ai"]["confidence_threshold"]),
        )
        S["ai"]["contributes_to_score"] = st.toggle(
            "Let AI contribute to the score (not yet implemented)",
            value=bool(S["ai"]["contributes_to_score"]), disabled=True,
        )

    with tabs[4]:
        st.subheader("Telegram")
        S["telegram"]["enabled"] = st.toggle(
            "Telegram alerts enabled", value=bool(S["telegram"]["enabled"]),
        )
        S["telegram"]["bot_token"] = st.text_input(
            "Bot token", value=S["telegram"]["bot_token"], type="password",
        )
        S["telegram"]["chat_id"] = st.text_input("Chat ID", value=S["telegram"]["chat_id"])
        levels = ["WEAK BUY", "BUY", "STRONG BUY", "EXTREME BUY"]
        S["telegram"]["min_signal_level"] = st.selectbox(
            ...
            index=levels.index(S["telegram"]["min_signal_level"])
            if S["telegram"]["min_signal_level"] in levels else 2,
        ).split(" /")[0].strip()
        S["telegram"]["cooldown_minutes"] = st.number_input(
            "Alert cooldown (minutes)", min_value=0,
            value=int(S["telegram"]["cooldown_minutes"]), step=5,
        )
        st.caption("Telegram uses the same signal engine as the dashboard.")
        if st.button("📨 Send test message"):
            ok, detail = send_telegram_message(
                S["telegram"]["bot_token"], S["telegram"]["chat_id"],
                "👑 SEKWAILA OMEGA X — test message. Telegram is wired up correctly.",
            )
            (st.success if ok else st.error)(detail)

    with tabs[5]:
        st.subheader("Data")
        S["data"]["selected_assets"] = st.multiselect(
            "Active pairs", list(ASSETS.keys()), default=S["data"]["selected_assets"],
        )
        st.divider()
        st.subheader("Price provider")
        st.success(
            "Manual calibration is disabled in the active price path. "
            "Current prices are supplied by Yahoo Finance. MT5/broker feed "
            "integration is the next step for true broker-price parity."
        )
        st.caption(
            "Do not use an offset to force a broker match. Different instruments "
            "such as GC=F vs XAUUSD can have structurally different prices."
        )

    with tabs[6]:
        st.subheader("Display")
        S["display"]["compact_mode"] = st.toggle(
            "Compact card mode", value=bool(S["display"]["compact_mode"]),
        )
    ...
    results = fetch_all(
        selected_assets if symbol in selected_assets else selected_assets + [symbol],
        min_tf, min_score, min_rr, refresh_seconds,
    )
    ...
    if not result or not result.get("ok"):
        ...
        st.warning(
            "Signal generation is BLOCKED. The dashboard will not generate, "
            "display, or count a BUY/SELL signal while a required timeframe is unavailable."
        )
        st.caption(
            result.get("reason", "No valid market-data result returned.")
            if result else "No result returned."
        )
        if result and result.get("data_integrity"):
            st.subheader("Timeframe data status")
            st.dataframe(
                pd.DataFrame([
                    {"Timeframe": tf, "Status": status}
                    for tf, status in result["data_integrity"].items()
                ]),
                width="stretch",
                hide_index=True,
            )
        ...
        st.caption(
            '"Entry" is the last CLOSED candle used by the signal engine. '
            '"Price" is the current quote from the live-price provider. '
            'No manual calibration is being applied.'
        )
        chips = "".join([
        ...
        if result.get("bias") in ("BUY", "SELL"):
            ...
            position = calculate_position_size_for_symbol(
                symbol, sizing_usd, S["risk"]["risk_pct"],
                result.get("entry", 0), result.get("stop", 0),
            )
        ...
        if available_tfs:
            chosen_tf = st.radio(
                "Timeframe", available_tfs, index=len(available_tfs) - 1,
                horizontal=True, key=f"tf_{symbol}",
            )
            df = result["data"][chosen_tf].tail(180)
            fig = go.Figure(go.Candlestick(
                x=df.index, open=df["Open"], high=df["High"],
                low=df["Low"], close=df["Close"], name=chosen_tf,
                increasing_line_color="#22e6a3", decreasing_line_color="#ff4d6a",
            ))
            if result.get("bias") in ("BUY", "SELL"):
                for name, val, color in [
                    ("Entry", result.get("entry"), "#8592a8"),
                    ("Stop", result.get("stop"), "#ff4d6a"),
                    ("TP1", result.get("tp1"), "#22e6a3"),
                    ("TP2", result.get("tp2"), "#22e6a3"),
                    ("TP3", result.get("tp3"), "#22e6a3"),
                ]:
                    v = num(val)
                    if v > 0:
                        fig.add_hline(
                            y=v, line_dash="dash", line_color=color,
                            annotation_text=f"{name} {price(v)}",
                            annotation_font_color=color,
                        )
            vwap = num(result.get("vwap_val"))
            if vwap > 0:
                fig.add_hline(
                    y=vwap, line_dash="dot", line_color="#ffb238",
        ...
        with st.expander("🛡️ Data integrity"):
            ...
            if integrity:
                st.dataframe(
                    pd.DataFrame([
                        {"Timeframe": tf, "Status": s}
                        for tf, s in integrity.items()
                    ]),
                    width="stretch", hide_index=True,
                )
            st.caption(
                "The engine removes the currently forming candle before indicator and structure calculations. "
                "A missing required timeframe blocks signal generation; unavailable data is never treated as neutral/valid."
            )
    ...
    if not selected_assets:
        ...
        for i, (_, symbol, result, level) in enumerate(ranked):
            ...
            with cols[i % 2]:
                if not result or not result.get("ok"):
                    unavailable_reason = (result or {}).get(
                        "reason", "No valid market-data result."
                    )
                    st.markdown(
                        f"""<div class="sig-card neutral">
                            <div class="top-row">
                              <div>
                                <div class="sig-symbol">{symbol}</div>
                                <div class="sig-ticker">DATA STATUS</div>
                              </div>
                              <span class="tf-pill neutral">DATA UNAVAILABLE</span>
                            </div>
                            <div class="sig-meta">
                              <span>{unavailable_reason[:120]}</span>
                            </div>
                        </div>""",
                        unsafe_allow_html=True,
                    )
                    ...
                    agreement = max(int(result.get("bull_tf_count", 0)), int(result.get("bear_tf_count", 0)))
                    tf_count = len(result.get("tf_biases", {})) or len(TF_ORDER)
                    card_live = get_live_price(symbol)
...
st.markdown("---")
st.caption(
    f"SEKWAILA OMEGA X · signals/signal_engine.py is the single source of truth "
    f"for dashboard and Telegram · {now_str}"
)
