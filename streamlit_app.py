...
The dashboard and Telegram worker both call generate_omega_signal()
from engine.py.

There is only one signal engine.
"""
...
<style>
    ...
    @import url(
        'https://fonts.googleapis.com/css2?family=Cinzel:wght@400;600;700&family=Inter:wght@300;400;600&display=swap'
    );
...
st.markdown(
    "<h1 class='title-cinzel' style='text-align:center; margin:0;'>"
    "SEKWAILA OMEGA X"
    "</h1>",
    unsafe_allow_html=True,
)
...
st.markdown(
    "<p style='text-align:center; color:#dfb15b; "
...
if page == "Dashboard":
    ...
    with col_center:
        ...
        # Only show levels as trade levels when active.
        ...
        if (
            ...
            elif "BEARISH" in res["ob_type"]:
                ob_color = (
                    "rgba(255,82,82,0.15)"
                )
            else:
                ob_color = (
                    "rgba(223,177,91,0.10)"
                )
        ...
        swing_hi = res["pd_info"][
            "swing_high"
        ]

        swing_lo = res["pd_info"][
            "swing_low"
        ]
...
# ==============================================================================
    ...
    st.caption(
        "Every row uses the same generate_omega_signal() "
        "used by Telegram."
    )
...
# ==============================================================================
    ...
    if not r["ok"]:
        ...
        narrative = (
            f"{r['symbol']} shows "
            f"{r['structure']} on 15M, "
            f"in a {r['regime']['regime']} regime "
            f"(ADX {r['regime']['adx']}). "
            f"{r['sweep_detail']}. "
            f"Price is in {r['pd_zone']}. "
...
# ==============================================================================
    ...
    if j_submit:
        ...
        if not j_notes.strip():
            ...
            st.warning(
                "Please add setup/execution notes."
            )
...
# ==============================================================================
    ...
    if not perf["has_data"]:
        ...
        st.info(
            ...
            "Log outcomes in Trade Journal to see real stats here."
        )
...
# ==============================================================================
    ...
    st.caption(
        "These settings are shared by the dashboard and worker."
    )
    ...
    st.markdown(
        "- Score is a rules-based heuristic and is not a backtested win rate.\n"
        "- No live economic-news filter is wired in.\n"
        "- Dashboard does not place broker trades.\n"
        "- Position sizing is reference sizing and must be matched to the actual broker contract specification."
    )
