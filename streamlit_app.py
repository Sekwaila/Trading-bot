...
st.set_page_config(
    ...
    page_icon="⚡",
    layout="wide",
    initial_sidebar_state="expanded" # Keeps settings visible
)
...
show_adx = st.sidebar.checkbox("Show ADX Power", value=True)
show_targets = st.sidebar.checkbox("Show TP / SL Lines", value=True)

# -----------------------------------------------------------------------------
# 3. MAIN DASHBOARD CONTROLS
# -----------------------------------------------------------------------------
TICKER_MAP = {
    "XAUUSD": "GC=F",
    "BTCUSD": "BTC-USD",
...
macd_trend = "BULL" if last_bar['MACD'] > last_bar['MACDs'] else "BEAR"

# -----------------------------------------------------------------------------
# 4. CUSTOM METRICS MATRIX
# -----------------------------------------------------------------------------
matrix_html = f"""
...
if show_adx:
    matrix_html += f"<div class='ind-row'><span>ADX Power</span><b>{adx_val:.1f}</b></div>"
