...
def chart_for(res):
    ...
    df=df.tail(180)
    fig=go.Figure(go.Candlestick(x=df.index,open=df["Open"],high=df["High"],low=df["Low"],close=df["Close"],name="15M"))
    if res.get("bias") in ("BUY","SELL"):
        for name,val in [("Entry",res.get("entry")),("Stop",res.get("stop")),("TP1",res.get("tp1")),("TP2",res.get("tp2")),("TP3",res.get("tp3"))]:
            v=num(val)
            if v>0: fig.add_hline(y=v,line_dash="dash",annotation_text=f"{name} {price(v)}")
    vwap=num(res.get("vwap_val"))
    if vwap>0: fig.add_hline(y=vwap,line_dash="dot",annotation_text=f"VWAP {price(vwap)}")
    fig.update_layout(template="plotly_dark",height=540,margin=dict(l=10,r=10,t=30,b=10),xaxis_rangeslider_visible=False)
    return fig
...
if bias=="BUY": st.success("🟢 BUY SETUP")
elif bias=="SELL": st.error("🔴 SELL SETUP")
...
with q1: st.metric("ENTRY",price(result.get("entry"))); st.metric("TP1",price(result.get("tp1")))
with q2: st.metric("STOP",price(result.get("stop"))); st.metric("TP2",price(result.get("tp2")))
with q3:
    rr=num(result.get("rr")); st.metric("R:R to TP2",f"{rr:.2f}" if rr else "—"); st.metric("TP3",price(result.get("tp3")))

st.markdown("## 📉 PRIMARY 15M CHART")
fig=chart_for(result)
if fig: st.plotly_chart(fig,width="stretch",config={"displaylogo":False,"responsive":True})
else: st.info("15M chart data unavailable.")

st.markdown("## 🧭 MULTI-TIMEFRAME AGREEMENT")
rows=[{"Timeframe":tf,"Bias":result.get("tf_biases",{}).get(tf,"NEUTRAL"),"Structure":result.get("tf_structures",{}).get(tf,"NONE")} for tf in result.get("tf_biases",{})]
