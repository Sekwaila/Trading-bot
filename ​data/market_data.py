...
def fetch_institutional_data(symbol: str = config.SYMBOL) -> Tuple[Dict[str, Optional[pd.DataFrame]], Dict[str, str]]:
    ...
    for tf_label, (period, interval) in config.TIMEFRAMES.items():
        try:
            # Enforce multi_level_index=False to prevent MultiIndex columns in modern yfinance
            df = yf.download(
                symbol, 
                period=period, 
                interval=interval, 
            ...
            # Safety fallback for older yfinance versions
    ...
    for name, ticker in symbols.items():
        try:
            ...
            if not d.empty and "Close" in d.columns:
                # Explicitly extract Close as a Series to safely append
                df_closes[name] = d["Close"].squeeze()
        except Exception as e:
            logger.warning(f"Failed correlation fetch for {name}: {e}")
